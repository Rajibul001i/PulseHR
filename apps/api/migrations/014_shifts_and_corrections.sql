...
-- SQLite dialect; see migrations-postgres/014_shifts_and_corrections.sql.
...
-- Notifications gain correction and shift types. SQLite cannot alter a CHECK constraint, so
-- the table is rebuilt with the same columns.
CREATE TABLE notification_v2 (
  id              TEXT PRIMARY KEY,
  organisation_id TEXT NOT NULL REFERENCES organisation(id),
  user_id         TEXT NOT NULL REFERENCES app_user(id),
  type            TEXT NOT NULL CHECK (type IN ('LEAVE_PENDING', 'LEAVE_DECIDED',
                    'CORRECTION_PENDING', 'CORRECTION_DECIDED', 'SHIFT_ASSIGNED')),
...
INSERT INTO notification_v2 (id, organisation_id, user_id, type, message, entity_type, entity_id, read_at, created_at)
  SELECT id, organisation_id, user_id, type, message, entity_type, entity_id, read_at, created_at FROM notification;
