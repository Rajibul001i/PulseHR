...
-- Enforce immutability at the database level, not by convention (NFR-9). SQLite expresses
-- this as a bare `CREATE TRIGGER ... BEGIN SELECT RAISE(ABORT, ...) END`; Postgres triggers
-- must invoke a function, so the same rule becomes a small PL/pgSQL function shared by both
-- tables' BEFORE UPDATE triggers (this is the pattern docs/03-data-model.md's §3 shows).
...
CREATE OR REPLACE FUNCTION reject_payslip_line_update() RETURNS trigger AS $$
BEGIN
  RAISE EXCEPTION 'payslip_line is immutable';
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER payslip_line_is_immutable
BEFORE UPDATE ON payslip_line
FOR EACH ROW EXECUTE FUNCTION reject_payslip_line_update();
